...
async def post_audio(file: UploadFile = File(...)):
    ...
    # Decode audio en utilisant le chemin complet du fichier
    message_decoded = convert_audio_to_text(audio_input)

    # Guard: Ensure output
    if not message_decoded:
        raise HTTPException(status_code=400, detail=file.filename)
    ...
    if not chat_response:
        raise HTTPException(status_code=400, detail=message_decoded)
